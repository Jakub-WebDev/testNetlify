...
export default function Latest() {
  ...
  const [displayedProducts, setDisplayedProducts] = useState(
    products.filter((product) => product.id >= 1 && product.id <= 6)
  );
  ...
  const handleButtonClick = (type) => {
    if (type === "newArrival") {
      ...
      setDisplayedProducts(
        products.filter((product) => product.id >= 7 && product.id <= 12)
      );
    } else if (type === "featured") {
      setDisplayedProducts(
        products.filter((product) => product.id >= 13 && product.id <= 18)
      );
    } else if (type === "specialOffer") {
      // Filter products with cashback greater than 0.10 and display the first 6
      const specialOfferProducts = products.filter(
        (product) => product.cashback > 0.1
      );
      setDisplayedProducts(specialOfferProducts.slice(0, 6));
    }
  };
  ...
  return (
    <div className={styles.latestContainer}>
      ...
      <div className={styles.navigation}>
        <button onClick={() => handleButtonClick("newArrival")}>
          New Arrival
        </button>
        <button onClick={() => handleButtonClick("bestSeller")}>
          Best Seller
        </button>
        <button onClick={() => handleButtonClick("featured")}>Featured</button>
        <button onClick={() => handleButtonClick("specialOffer")}>
          Special Offer
        </button>
      </div>
      <div className={styles.products}>
        {displayedProducts.map((product, index) => (
          ...
          >
            {product.cashback > 0.1 && (
              <img src={Sale} alt="sale icon" className={styles.saleImage} />
            )}
            <div className={styles.smallButtons}>
              <AddToCartButton product={product} className={styles.smallButton}>
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
            ...
            <div className={styles.details}>
              ...
              <p className={styles.productNewPrice}>${product.newPrice}.00</p>
            </div>
          </Link>
        ))}
      </div>
