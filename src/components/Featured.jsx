...
export default function Featured() {
  ...
  const currentFeatureds = (index) => {
    setCurrentIndex(index);
  };

  const featuredOffset = -currentIndex * 100;

  return (
    <div className={styles.featuredContainer}>
      <h1 className={styles.featuredTitle}>Featured Products</h1>
      <div
        className={styles.featureds}
        style={{
          transform: `translateX(${featuredOffset}%)`,
        }}
      >
        {[
          ["1", "2", "3", "4"],
          ...
          <div key={index} className={styles.featured}>
            {products
              .filter((product) => group.includes(String(product.id)))
              .map((product) => (
                <div key={product.id} className={styles.featuredContent}>
                  ...
                  <div className={styles.smallButtons}>
                    ...
                    <a
                      href={`http://localhost:5173/${product.image}`}
                      className={styles.smallButton}
                      target="_blank"
                    >
                      <svg
      ...
      <div className={styles.dots}>
        <span
          ...
          onClick={() => currentFeatureds(0)}
        ></span>
        <span
          className={`${styles.dot} ${currentIndex === 1 ? styles.active : ""}`}
          onClick={() => currentFeatureds(1)}
        ></span>
        <span
          className={`${styles.dot} ${currentIndex === 2 ? styles.active : ""}`}
          onClick={() => currentFeatureds(2)}
        ></span>
        <span
          className={`${styles.dot} ${currentIndex === 3 ? styles.active : ""}`}
          onClick={() => currentFeatureds(3)}
        ></span>
      </div>
    </div>
  );
